...
// FK chain: user_sessions.id ← user_profiles.id ← user_events.user_id
// Both the session and profile rows must exist before user_events can be
// written. Idempotent via upsert(ignoreDuplicates) so a re-run is a no-op.
async function ensureProfileRow(userId) {
  ...
  const { error: sessionError } = await supabase
    .from('user_sessions')
    .upsert(
      { id: userId, expires_at: expiresAt },
      { onConflict: 'id', ignoreDuplicates: true }
    );
  if (sessionError) {
    console.error('Failed to ensure user_sessions row:', sessionError);
    throw new Error(`Failed to bootstrap user session: ${sessionError.message}`);
  }
